...
![Metuur в терминале Windows](docs/metuur-interface.png)
...
- локальный micro-AI понимает обычные фразы (`запусти файл`, `собери проект`,
  `проверь тесты`), опечатки и ввод в неверной раскладке;
- AI работает полностью офлайн, анализирует только структуру текущего Go-проекта
  и запоминает удачные команды; модель занимает лишь несколько килобайт;
...
## Локальный AI намерений

Это не тяжёлая языковая модель и не облачный чат. Встроенный интерпретатор
определяет желаемое действие, после чего использует активный файл VS Code,
`package main`, `go.mod`, тесты и пакеты открытой папки для построения команды.
Ничего не скачивается и не отправляется в интернет.

Примеры: `запусти открытый файл`, `собери проект`, `отформатируй весь код`,
`проверь тесты`, `обнови зависимости`, `очисти кэш`, `проверь уязвимости`.
Поддерживается и случайная английская раскладка: `pfgencnb afqk`.

После успешного выполнения Metuur локально запоминает команду и лучше ранжирует
её в той же папке. Состояние можно посмотреть командой `metuur ai status`, а
отключить адаптацию — параметром `"localAIEnabled": false` в конфигурации.
...
Перед первой публичной публикацией изменённой версии автору необходимо отправить
уведомление через [Issue](https://github.com/wertyy111/Metuur/issues) с адресом
версии и описанием существенных изменений. Официальная ветка `main` защищена:
изменения принимаются только через Pull Request и проверку владельца.
...
архитектурный ориентир. Подробности: [проверка совместимости с IRIS](docs/IRIS-COMPATIBILITY.md).
